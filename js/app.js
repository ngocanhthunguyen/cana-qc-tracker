    ...
    return `<div class="batch-card mob-card">
      <div class="card-top">
        <div class="card-head-text">
          <div class="card-title">${esc(r.staff)}</div>
          <div class="card-subtitle">${esc(r.date)} · ${r.sessions} entr${r.sessions===1?'y':'ies'}</div>
        </div>
      </div>
...
function savePackingPlan(pickIds, meta){
  ...
  const plan = {
    ...
    shipToName: (meta && meta.shipToName) || (meta && meta.company) || '',
    shipToAddress: (meta && meta.shipToAddress) || '',
    shipFromAddress: (meta && meta.shipFromAddress) || PACKING_FROM_ADDRESS_DEFAULT,
...
function savePackingLabelPrintSettings(tier){
  ...
  const rot = document.getElementById('packingRotateChk');
  if(rot && (tier === 'box' || tier === 'pallet')){
    localStorage.setItem('cana_pkglabel_rotate_' + tier, rot.checked ? '1' : '0');
  }
}
function getPackingLabelRotate(tier){
  if(tier !== 'box' && tier !== 'pallet') return false;
  const v = localStorage.getItem('cana_pkglabel_rotate_' + tier);
...
function packingUnitLabelShape(type, unit, plan){
  ...
  const shipFromAddress = (plan && plan.shipFromAddress) || PACKING_FROM_ADDRESS_DEFAULT;
...
function buildZplPackingLabel(shape, sizeIn, dpi){
  if(shape.kind === 'bag') return buildZplBagLabel(shape, sizeIn, dpi);
  return buildZplShipLabel(shape, sizeIn, dpi);
}
function buildZplBagLabel(shape, sizeIn, dpi){
  const safe = (s)=> String(s || '').replace(/[^\x20-\x7E]/g, '').slice(0, 40);
  const pw = inchesToDots(sizeIn.w, dpi);
  const ll = inchesToDots(sizeIn.h, dpi);
  const margin = Math.max(12, Math.round(pw * 0.04));
  const innerW = pw - margin * 2;
  let y = margin;
  let zpl = '^XA^MMT^MNY^PW' + pw + '^LL' + ll + '^LH0,0^LT0\n';
  const headerFs = Math.max(22, Math.round(ll * 0.11));
  zpl += '^FO' + margin + ',' + y + '^A0N,' + headerFs + ',' + headerFs + '^FB' + innerW + ',1,0,C^FD' + safe(shape.header) + '^FS\n';
  y += headerFs + 4;
  if(shape.subheader){
    const subFs = Math.max(14, Math.round(ll * 0.07));
    zpl += '^FO' + margin + ',' + y + '^A0N,' + subFs + ',' + subFs + '^FB' + innerW + ',1,0,C^FD' + safe(shape.subheader) + '^FS\n';
    y += subFs + 6;
  }
  zpl += '^FO' + margin + ',' + y + '^GB' + innerW + ',2,2^FS\n';
  y += 10;
  const fieldFs = Math.max(18, Math.round(ll * 0.09));
  ...
  const bh = Math.max(40, Math.round(ll * 0.18));
  const bx = margin + Math.max(0, Math.round((innerW - barW) / 2));
  y += 4;
  zpl += '^FO' + bx + ',' + y + '^BY' + moduleW + ',3,' + bh + '^BCN,' + bh + ',N,N,N^FD' + id + '^FS\n';
  y += bh + 6;
  const codeFs = Math.max(18, Math.round(ll * 0.08));
  zpl += '^FO' + margin + ',' + y + '^A0N,' + codeFs + ',' + codeFs + '^FB' + innerW + ',1,0,C^FD' + id + '^FS\n';
  y += codeFs + 4;
  if(shape.counterText){
    const cFs = Math.max(16, Math.round(ll * 0.07));
    zpl += '^FO' + margin + ',' + y + '^A0N,' + cFs + ',' + cFs + '^FB' + innerW + ',1,0,C^FD' + safe(shape.counterText) + '^FS\n';
  }
  zpl += '^PQ1^XZ\n';
  return zpl;
}
/** Shipping-style box/pallet ZPL. sizeIn is physical stock (usually 4×6).
 *  When rotate=true, content is composed as landscape (6×4) then placed with ^A0R/^BCR. */
function buildZplShipLabel(shape, sizeIn, dpi){
  ...
  const physW = inchesToDots(sizeIn.w, dpi);
  const physH = inchesToDots(sizeIn.h, dpi);
  const DW = rotate ? physH : physW;
  const DH = rotate ? physW : physH;
  const margin = Math.max(16, Math.round(Math.min(DW, DH) * 0.03));
  ...
  const map = (dx, dy, alongX)=>{
    if(!rotate) return { x: dx, y: dy };
    const fl = alongX || 0;
    return { x: dy, y: Math.max(0, DW - dx - fl) };
  };
  const text = (dx, dy, fs, str, opts)=>{
    opts = opts || {};
    const maxW = opts.maxW || innerW;
    const align = opts.align || 'L';
    if(rotate){
      const p = map(dx, dy, fs);
      return '^FO' + p.x + ',' + p.y + '^A0R,' + fs + ',' + fs + '^FD' + str + '^FS\n';
    }
    if(align === 'C'){
      return '^FO' + dx + ',' + dy + '^A0N,' + fs + ',' + fs + '^FB' + maxW + ',1,0,C^FD' + str + '^FS\n';
    }
    return '^FO' + dx + ',' + dy + '^A0N,' + fs + ',' + fs + '^FD' + str + '^FS\n';
  };
  const hline = (dx, dy, w, thick)=>{
    if(rotate){
      const p = map(dx, dy, w);
      return '^FO' + p.x + ',' + p.y + '^GB' + thick + ',' + w + ',' + thick + '^FS\n';
    }
    return '^FO' + dx + ',' + dy + '^GB' + w + ',' + thick + ',' + thick + '^FS\n';
  };
  const rect = (dx, dy, w, h, thick)=>{
    if(rotate){
      const p = map(dx, dy, w);
      return '^FO' + p.x + ',' + p.y + '^GB' + h + ',' + w + ',' + thick + '^FS\n';
    }
    return '^FO' + dx + ',' + dy + '^GB' + w + ',' + h + ',' + thick + '^FS\n';
  };

  let zpl = '^XA^MMT^MNY^PW' + physW + '^LL' + physH + '^LH0,0^LT0\n';
  let y = margin;

  const headerFs = Math.max(32, Math.round(DH * 0.08));
  zpl += text(margin, y, headerFs, safe(shape.header, 40), { maxW: innerW, align: 'C' });
  y += headerFs + 8;
  if(shape.subheader){
    const subFs = Math.max(20, Math.round(DH * 0.05));
    zpl += text(margin, y, subFs, safe(shape.subheader, 36), { maxW: innerW, align: 'C' });
    y += subFs + 8;
  }
  zpl += hline(margin, y, innerW, 3);
  y += 14;

  const titleFs = Math.max(24, Math.round(DH * 0.055));
  const addrFs = Math.max(18, Math.round(DH * 0.042));
  const maxAddrChars = Math.max(20, Math.floor(leftW / (addrFs * 0.52)));
  if(shape.fromTo){
    zpl += text(margin, y, titleFs, safe('From: ' + shape.fromTo.from, 42));
    zpl += text(rightX, y, titleFs, safe('To: ' + shape.fromTo.to, 42));
    y += titleFs + 6;
    const fromLines = wrapLabelWords(shape.fromTo.fromAddress || '', maxAddrChars).slice(0, 3);
    const toLines = wrapLabelWords(shape.fromTo.toAddress || '', maxAddrChars).slice(0, 3);
    const rows = Math.max(fromLines.length, toLines.length, 1);
    for(let i=0;i<rows;i++){
      if(fromLines[i]) zpl += text(margin, y, addrFs, safe(fromLines[i], 44));
      if(toLines[i]) zpl += text(rightX, y, addrFs, safe(toLines[i], 44));
      y += addrFs + 4;
    }
    y += 8;
  }
  ...
  const fieldFs = Math.max(28, Math.round(DH * 0.06));
  (shape.fields || []).forEach(f=>{
    zpl += text(margin, y, fieldFs, safe(f.label + ' : ' + f.value, 48));
    y += fieldFs + 10;
  });
  y += 8;
  zpl += hline(margin, y, innerW, 3);
  ...
  const footTop = y;
  const hcFs = Math.max(24, Math.round(DH * 0.055));
  const hcBoxH = hcFs + 24;
  zpl += rect(margin, footTop, leftW, hcBoxH, 3);
  zpl += text(margin + 10, footTop + 12, hcFs, safe('HANDLE PACKAGE CAREFULLY', 32));

  const cFs = Math.max(32, Math.round(DH * 0.07));
  zpl += text(rightX, footTop, cFs, safe(shape.counterText || '', 16), { maxW: rightW, align: 'C' });
  let by = footTop + cFs + 10;
  const id = safe(shape.code, 20);
  let moduleW = 3;
  let barW = estimateCode128Dots(id.length, moduleW);
  if(barW > rightW){ moduleW = 2; barW = estimateCode128Dots(id.length, moduleW); }
  if(barW > rightW){ moduleW = 1; barW = estimateCode128Dots(id.length, moduleW); }
  const bh = Math.max(60, Math.round(DH * 0.14));
  if(rotate){
    const p = map(rightX + Math.max(0, Math.round((rightW - barW) / 2)), by, bh);
    zpl += '^FO' + p.x + ',' + p.y + '^BY' + moduleW + ',3,' + bh + '^BCR,' + bh + ',N,N,N^FD' + id + '^FS\n';
  } else {
    const bx = rightX + Math.max(0, Math.round((rightW - barW) / 2));
    zpl += '^FO' + bx + ',' + by + '^BY' + moduleW + ',3,' + bh + '^BCN,' + bh + ',N,N,N^FD' + id + '^FS\n';
  }
  by += bh + 8;
  const codeFs = Math.max(22, Math.round(DH * 0.05));
  zpl += text(rightX, by, codeFs, id, { maxW: rightW, align: 'C' });
  by += codeFs + 6;
  if(shape.footNote){
    const fnFs = Math.max(20, Math.round(DH * 0.045));
    zpl += text(rightX, by, fnFs, safe(shape.footNote, 24), { maxW: rightW, align: 'C' });
  }
...
function openPrintPackingLabelsModal(planId){
  ...
  <div class="overlay" id="overlay">
    <div class="modal modal-wide plant-print-modal">
      ...
      <div class="plant-print-help">
        ...
        <p class="sub" style="margin:4px 0 0;">Box/pallet default is 4×6 stock, large fonts, rotated landscape (like your paper sample). Bags stay smaller and upright.</p>
      </div>
      <div class="form-grid">
        ...
        <div class="field"><label>Grade</label>
          <input id="printGrade" value="${esc(plan.grade || '')}" placeholder="e.g. A" maxlength="8"></div>
        <div class="field full"><label>Deliver to — address</label>
          <textarea id="printShipToAddress" rows="2" placeholder="Customer delivery address">${esc(plan.shipToAddress || '')}</textarea></div>
        <div class="field full"><label>From address</label>
          <textarea id="printShipFromAddress" rows="2">${esc(plan.shipFromAddress || PACKING_FROM_ADDRESS_DEFAULT)}</textarea></div>
        ...
        <div class="field full"><label class="chk"><input type="checkbox" id="packingRotateChk" ${getPackingLabelRotate(tier)?'checked':''}> Rotate box/pallet label 90° (landscape on 4×6 stock)</label></div>
      </div>
  ...
  const applyShipFieldsToPlan = ()=>{
    plan.shipToName = (document.getElementById('printShipToName')||{}).value.trim();
    plan.shipToAddress = (document.getElementById('printShipToAddress')||{}).value.trim();
    plan.shipFromAddress = (document.getElementById('printShipFromAddress')||{}).value.trim() || PACKING_FROM_ADDRESS_DEFAULT;
    plan.grade = (document.getElementById('printGrade')||{}).value.trim();
    if(plan.shipToName) plan.company = plan.shipToName;
    onDataChanged();
  };
  const refreshSheet = ()=>{
  ...
  bindTestPrintButtons();
  const rotChk = root.querySelector('#packingRotateChk');
  if(rotChk){
    rotChk.disabled = tier === 'bag';
    rotChk.onchange = ()=>{
      if(tier === 'bag') return;
      localStorage.setItem('cana_pkglabel_rotate_' + tier, rotChk.checked ? '1' : '0');
      refreshSheet();
    };
  }
  ...
  root.querySelectorAll('#packingTierToggle button').forEach(btn=>{
    btn.onclick = ()=>{
      ...
      if(rotChk){
        rotChk.disabled = tier === 'bag';
        rotChk.checked = tier === 'bag' ? false : getPackingLabelRotate(tier);
      }
  ...
  root.querySelector('#btnClosePrint').onclick = ()=>{ applyShipFieldsToPlan(); modalDirty = false; closeModal(); };
  root.querySelector('#overlay').onclick = (e)=>{ if(e.target.id==='overlay'){ applyShipFieldsToPlan(); modalDirty = false; closeModal(); } };
...
function openPackingPreviewModal(pickIds){
  ...
  <div class="overlay" id="overlay">
    <div class="modal modal-wide">
      ...
      <div class="form-grid">
        <div class="field"><label>Deliver to — company / customer name</label>
          <input list="packingCompanyList" id="packingCompanyInput" placeholder="e.g. BLS Wholesalers Pty Ltd" required>
        ...
        <div class="field full"><label>Label / reference (optional)</label>
  ...
  root.querySelector('#btnConfirmPacking').onclick = ()=>{
    ...
    const shipToAddress = root.querySelector('#packingShipToAddress').value.trim();
    const shipFromAddress = root.querySelector('#packingShipFromAddress').value.trim() || PACKING_FROM_ADDRESS_DEFAULT;
    if(!company){ alert('Enter the customer / company name for the To: line on box labels.'); return; }
    if(!shipToAddress){ alert('Enter the delivery address for the customer (To: address on box labels).'); return; }
    const plan = savePackingPlan(pickIds, {
      company, label, grade,
      shipToName: company,
      shipToAddress,
      shipFromAddress
  ...
  return records.map(rec=>{
    ...
    if(isCana){
      return `<div class="batch-card mob-card">
        <div class="card-top">
          <div class="card-head-text">
            <div class="card-title">${esc(rec.strain||'—')}</div>
            <div class="card-subtitle">${esc(rec.room||'—')} · ${esc(rec.date||'—')}</div>
          </div>
        ...
        <div class="card-meta">
          ...
          <span>Stems ${fmtWeight(rec.stemsG)}</span>
          <span>Waste ${fmtWeight(rec.wasteG)}</span>
        </div>
        ${actions}
      </div>`;
    }
    if(isDaily){
      ...
      return `<div class="batch-card mob-card">
        <div class="card-top">
          <div class="card-head-text">
            <div class="card-title">${esc(rec.date||'—')}</div>
            <div class="card-subtitle">${esc(rec.trimmedBy||'—')}</div>
          </div>
          <span class="status-chip ${statusCls}">${esc(statusShort)}</span>
        </div>
        <div class="card-stats">
        ...
        <div class="card-meta"><span>${esc(notes)}</span></div>
        ${actions}
      </div>`;
    }
    return `<div class="batch-card mob-card">
      <div class="card-top">
        <div class="card-head-text">
          <div class="card-title">${esc(rec.strain||'—')}</div>
          <div class="card-subtitle"><span class="batch-id">${esc(rec.batchId||'—')}</span> · ${esc(rec.sourceFarm||'—')}</div>
        </div>
        <span class="status-chip ${statusCls}">${esc(statusShort)}</span>
      </div>
      <div class="card-stats">
...
function renderCureSessionsCardList(sessions){
  return sessions.map(s=>{
    const days = daysInCure(s);
    const target = num(s.targetDays);
    const dayLabel = days !== null ? days + 'd' + (target !== null ? ' / ' + target + 'd' : '') : '—';
    const logs = getCureLogsForSession(s.id).length;
    const sc = cureStatusClass(s.status);
    const strains = (s.strains||'—').slice(0, 80) + ((s.strains||'').length > 80 ? '…' : '');
    return `<div class="batch-card mob-card">
      <div class="card-top">
        <div class="card-head-text">
          <div class="card-title">${esc(s.room||'—')}</div>
          <div class="card-subtitle">${esc(strains)}</div>
        </div>
        <span class="status-chip ${sc}">${esc((s.status||'—').split(' / ')[0])}</span>
      </div>
      <div class="card-stats">
        <div class="stat"><div class="n">${dayLabel}</div><div class="t">Days</div></div>
        <div class="stat"><div class="n">${logs}</div><div class="t">Logs</div></div>
        <div class="stat"><div class="n" style="font-size:12px;">${esc(s.startDate||'—')}</div><div class="t">Start</div></div>
      </div>
      <div class="card-meta"><span>${esc(s.assignedTo||'—')}</span></div>
      <div class="action-group">
        <button type="button" class="small" data-view-cure-logs="${esc(s.id)}">Logs</button>
...
function renderCureLogCardList(logs){
  return logs.map(l=>{
    const sess = getCureSession(l.sessionId);
    const desc = (l.description||'—').slice(0, 100) + ((l.description||'').length > 100 ? '…' : '');
    return `<div class="batch-card mob-card">
      <div class="card-top">
        <div class="card-head-text">
          <div class="card-title">${esc((l.action||'—').split(' / ')[0])}</div>
          <div class="card-subtitle">${esc(l.room||sess?.room||'—')} · ${esc(l.date||'—')} ${esc(formatCureLogTime(l.time))}</div>
        </div>
      </div>
      <div class="card-stats">
        <div class="stat"><div class="n">${l.minutes ? fmtNum(l.minutes, 0) : '—'}</div><div class="t">Mins</div></div>
        <div class="stat"><div class="n" style="font-size:12px;">${esc(l.doneBy||'—')}</div><div class="t">By</div></div>
      </div>
      <div class="card-meta"><span>${esc(desc)}</span></div>
      <div class="action-group">
...
function renderCureSessionsTable(sessions){
  ...
  <div class="card-list">${renderCureSessionsCardList(sessions)}</div>`;
}
function renderCureLogTable(logs, opts){
  ...
  <div class="card-list">${renderCureLogCardList(logs)}</div>`;
}
...
function renderCuringView(){
  ...
  main.innerHTML = `
    ...
    <div class="mob-section-label mobile-only">${isSessions ? 'Cure sessions' : 'Cure log'}</div>
...
function getFarmQcUsedByBatch(farm, strain){
  ...
  (state.exportPicks || []).filter(p=> p.farm === farm && canonicalStrainName(p.strain).toLowerCase() === norm)
    .forEach(p=> (p.allocations || []).forEach(a=>{
      ...
      const total = bigsG + popsG;
      const kgG = (num(a.kg) || 0) * 1000;
...
function allocateFarmQcPickFifo(farm, strain, bigsKgNeeded, popsKgNeeded){
  const { batches } = getFarmStrainQcAvailability(farm, strain);
  let remBigsG = Math.round((bigsKgNeeded||0)*1000), remPopsG = Math.round((popsKgNeeded||0)*1000);
  const allocations = [];
  for(const b of batches){
    if(remBigsG <= 0 && remPopsG <= 0) break;
    const takeBigs = Math.min(b.remBigsG, remBigsG);
    const takePops = Math.min(b.remPopsG, remPopsG);
    if(takeBigs > 0 || takePops > 0){
      allocations.push({ batchId: b.batchId, bigsG: takeBigs, popsG: takePops });
      remBigsG -= takeBigs;
      remPopsG -= takePops;
    }
  }
  return { allocations, shortfallBigsKg: Math.max(0, Number((remBigsG/1000).toFixed(3))), shortfallPopsKg: Math.max(0, Number((remPopsG/1000).toFixed(3))) };
}
...
function shipmentBatchStatus(s){
  ...
  if(s.source === 'farm_qc') return { label: 'From farm QC stock', cls: 'pass' };
...
function openBulkShipmentModal(){
  ...
  const newRow = ()=> ({ strain:'', bigsKg:'', popsKg:'', farm: farms[0] || '', gacpLicence: getLastGacpForFarm(farms[0] || ''), pickFromQc: false });
  ...
  function availHintText(row){
    const isOwnStock = row.farm === CANA_STOCK_SHIPMENT_SOURCE;
    if(isOwnStock || !row.pickFromQc) return '';
    if(!row.strain.trim()) return 'Type strain to see availability / พิมพ์สายพันธุ์เพื่อดูของที่มี';
    const avail = getFarmStrainQcAvailability(row.farm, row.strain.trim());
    return 'Avail: ' + fmtNum(avail.remainingBigsKg,2) + 'kg Bigs / ' + fmtNum(avail.remainingPopsKg,2) + 'kg Pops';
  }
  function rowHtml(row, i){
    ...
    return `<tr data-row="${i}">
      ...
      ${!isOwnStock ? `<label style="display:flex;align-items:center;gap:4px;font-size:10px;margin-top:4px;color:var(--muted);white-space:nowrap;"><input type="checkbox" class="ship-pick-qc" ${row.pickFromQc?'checked':''}> Already QC'd — pick from stock</label>` : ''}
      <div class="muted ship-avail-hint" style="font-size:10px;margin-top:2px;">${esc(availHintText(row))}</div>
      </td>
  ...
  function renderAll(){
    ...
    <div class="overlay" id="overlay">
      <div class="modal modal-wide">
        ...
        <p class="sub">Enter next month's expected shipment — pick strain, weight, and which farm it's from. By default this creates a new pending QC batch at that farm. Tick <b>"Already QC'd"</b> on a row to instead pick from that farm's finished flower that's already passed QC.<br>
        <span class="bi">กรอกแผนรับสินค้าเดือนถัดไป แยกตามสายพันธุ์และฟาร์ม — ค่าเริ่มต้นจะสร้างรายการรอ QC ใหม่ ติ๊ก "QC แล้ว" หากต้องการหยิบจากสต๊อกที่ QC ผ่านแล้วของฟาร์มนั้น</span></p>
  ...
  function updateAvailHint(tr, i){
    const hintEl = tr.querySelector('.ship-avail-hint');
    if(hintEl) hintEl.textContent = availHintText(draft.rows[i]);
  }
  function bindAll(){
    modalDirty = true;
    root.querySelector('#overlay').onclick = (e)=>{ if(e.target.id==='overlay') close(); };
    ...
    root.querySelectorAll('#shipDraftTable tbody tr').forEach(tr=>{
      ...
      tr.querySelector('.ship-strain').oninput = (e)=>{ draft.rows[i].strain = e.target.value; updateAvailHint(tr, i); };
      ...
      const pickQc = tr.querySelector('.ship-pick-qc');
      if(pickQc) pickQc.onchange = (e)=>{ draft.rows[i].pickFromQc = e.target.checked; updateAvailHint(tr, i); };
  ...
  function save(){
    ...
    validRows.forEach(r=>{
      ...
      if(r.pickFromQc){
        const { allocations, shortfallBigsKg, shortfallPopsKg } = allocateFarmQcPickFifo(r.farm, strain, bigsKg, popsKg);
        const shortfall = shortfallBigsKg + shortfallPopsKg;
        if(shortfall > 0) stockShortfalls.push(strain + ' @ ' + r.farm + ' short by ' + fmtNum(shortfall,2) + ' kg of finished QC stock');
        state.shipments.push({
          id: uid(), month, strain,
          bigsKg: bigsKg ? String(bigsKg) : '', popsKg: popsKg ? String(popsKg) : '', totalKg: String(totalKg),
          farm: r.farm, source: 'farm_qc', gacpLicence: (r.gacpLicence || '').trim(),
          notes: shortfall > 0 ? ('⚠ ' + fmtNum(shortfall,2) + ' kg short of finished QC stock at farm') : '',
          farmAllocations: allocations, linkedBatchId: '',
          createdBy: getCurrentUserName(), createdAt: new Date().toISOString()
        });
        return;
      }
...
function renderShipmentsTable(rows){
  ...
  const body = rows.map(s=>{
    ...
    return `<tr>
      ...
      <td><div class="action-group">
        ${s.source === 'stock' ? `<button class="small" data-goto-shipment-batch="${s.id}">View stock</button>` : (s.source === 'farm_qc' || s.linkedBatchId ? `<button class="small" data-goto-shipment-batch="${s.id}">Go to batch</button>` : '')}
...
function deleteShipment(id){
  ...
  } else if(s.source === 'farm_qc'){
    // Nothing to mutate — the picked kg is tracked only in this ledger entry,
    // so removing it automatically makes that kg available again at the farm.
  } else {
...
function renderCardList(records){
  return records.map(rec=>{
    ...
    const badgeClass = passFailBadgeClass(rec);
    const bid = getBatchId(rec, currentFarm);
    return `<div class="batch-card mob-card ${pend?'pending':''}">
      <div class="card-top">
        <div class="card-head-text">
          <span class="batch-id">${esc(bid)}</span>
          <div class="card-title">${esc(rec.strain||'No strain')}</div>
          <div class="card-subtitle">${esc(rec.date||'—')} · ${fmtNum(rec.grossWt)} g · ${esc(rec.receivedBy||'—')}</div>
        </div>
      ...
      <div class="action-group">
        <button class="small admin-only" data-edit-delivery="${rec.id}">Delivery</button>
